...
BEGIN
    ...
    FOR lst_i IN select ST_ConvexHull(rast) as ext, count(*) as num_tiles
        ...
        LOOP
            DELETE FROM postgis.evci
            WHERE rid = (SELECT rid FROM postgis.evci
                         WHERE id_acquisizione = id_acquisizione_in
                           AND   ST_ConvexHull(rast) = lst_i.ext
                         LIMIT 1);
...
-- calculate_vci2
-- Calculate VCI using tiles under given polygon.
--   this version will create an entry in tile_referenfes table in order to keep the
--   correlation between modis NDVI tiles and calculated VCI tiles
-- VERSION with tile_ref saving
...
BEGIN
    ...
    FOR lst_i IN select rast, ST_ConvexHull(rast) as ext
        ...
        LOOP
			...
			RAISE NOTICE 'saving tiles reference';

			INSERT INTO postgis.tile_references (id_acquisizione, tile_ref)
            VALUES
            (id_acquisizione_in, tile_ref_in);
