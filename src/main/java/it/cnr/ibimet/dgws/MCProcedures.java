...
public class MCProcedures implements SWH4EConst {
    ...
    /**
     * perform TCI calculation in multi threads
     *
     * @param nthread
     * @param year
     * @param doy
     ...
     * Returns : 0 OK
     */
    public int perform_tci_calculus( int nthread, int year, int doy) throws
            SQLException,Exception,InterruptedException{

        System.out.println("TCI Multithread start");
        ...
        startTime = new Timestamp(System.currentTimeMillis());
        System.out.println("TCI calculation, starting time: "+startTime);
        ...
        sqlString  = "select st_xmin(st_extent(convex_total)), st_ymin(st_extent(convex_total)), "+
                ...
                "where dtime = (select dtime from postgis.acquisizioni inner join postgis.imgtypes using (id_imgtype) "+
                "where imgtype = 'LST' and extract(doy from dtime)=? and extract(year from dtime)=? )) as foo";
        ...
        //checking acquisition entry
        System.out.print("TCI MC: Checking acquisition entry...");
        sqlString = "select id_acquisizione " +
                "from   postgis.acquisizioni inner join postgis.imgtypes using (id_imgtype) " +
                "where  imgtype = 'TCI' " +
                "and    extract(doy from dtime) = ? " +
        ...
        tdb.runPreparedQuery();
        if(tdb.next()){
            System.out.println("TCI MC: exists at "+doy+" "+year);
        }else{
            System.out.println("TCI MC: creating acquisition entry at "+doy+" "+year);
            sqlString = "INSERT INTO postgis.acquisizioni (dtime, id_imgtype)" +
                    " VALUES (to_timestamp('"+year+" "+doy+"', 'YYYY DDD')," +
        ...
        //deleting old tci
        System.out.println("deleting old tci "+year+" "+doy);
        sqlString = "delete from postgis.tci where id_acquisizione = " +
                "(select id_acquisizione from postgis.acquisizioni " +
                "inner join postgis.imgtypes using (id_imgtype) " +
                "where imgtype = 'TCI' and extract(year from dtime) = "+year+" " +
                "and extract(doy from dtime) = "+doy+ " )";
        tdb.setPreparedStatementRef(sqlString);
        tdb.performInsert();
        ...
        System.out.println("TCI -  colosing connection...");
        tdb.closeConnection();
        ...
        for (icount = xmin; icount <= xmax; icount+=w_part){
            for (jcount = ymin; jcount <= ymax; jcount+=h_part){
                ...
                System.out.println("TCi init instance n. "+i);
                FutureTask futureTask_n = new FutureTask(new RasterEngineCallable("TCI Thread-"+i,
                        icount,jcount,(icount+w_part),(jcount+h_part),"tci",year,doy));
        ...
        //Save calculated data into spi table
        System.out.println("TCI MC re-opening connection...");
        tdb = new TDBManager(db_context);

        System.out.println("TCI MC: deleting duplicated VCI tiles");

        sqlString="select * from postgis.deduplicate_tci("+doy+","+year+")";
        tdb.setPreparedStatementRef(sqlString);
        tdb.runPreparedQuery();
        if(tdb.next()){
            System.out.println("TCI MC deduplicated");


        }


        System.out.println("TCI MC closing connection...");
        tdb.closeConnection();


        if (!executor.isTerminated()) {
            System.err.println("TCI MC Attempt to shutdown executer");
            System.err.println("TCI MC using shutdownNow()");
        }
        executor.shutdownNow();
        endTime = new Timestamp(System.currentTimeMillis());
        System.out.println("TCI MC ending time: "+endTime);
